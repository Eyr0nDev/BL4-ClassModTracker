/*
Borderlands 4 — Class Mod Loot Tracker (v12 mobile-first header)
- Compact header on phones: short title + reset icon
- Full header on sm+ screens: logo + full title + info tooltip + Reset button
- Everything else kept modern/responsive
*/
...
const ROWS = ["White", "Green", "Blue", "Purple", "Orange"];

// Character icons (put these in /public)
const VH_ICONS = {
  Vex: "/Vex.png",
  Rafa: "/Rafa.png",
  Amon: "/Amon.png",
...
const rarityStyles = {
  White: { text: "text-slate-200", chip: "bg-slate-600/30 border-slate-500/40", dot: "bg-slate-300" },
  Green: { text: "text-green-400", chip: "bg-green-500/10 border-green-500/30", dot: "bg-green-400" },
  Blue: { text: "text-blue-400", chip: "bg-blue-500/10 border-blue-500/30", dot: "bg-blue-400" },
  Purple: { text: "text-violet-400", chip: "bg-violet-500/10 border-violet-500/30", dot: "bg-violet-400" },
  Orange: { text: "text-orange-400", chip: "bg-orange-500/10 border-orange-500/30", dot: "bg-orange-400" },
};
    ...
    <main className="min-h-screen bg-[#0b0b0d] text-slate-100 flex items-start justify-center p-4 sm:p-6 md:p-10">
      <div className="w-full max-w-5xl">
          ...
          <div className="hidden sm:grid grid-cols-[auto,1fr,auto] items-center gap-3">
            ...
            <h1 className="justify-self-center flex items-center gap-2 text-2xl md:text-3xl font-extrabold tracking-tight leading-tight">
              ...
              >
                ...
                <span className="pointer-events-none opacity-0 group-hover:opacity-100 transition
                                 bg-black text-white text-[11px] px-2 py-1 rounded absolute left-1/2 -translate-x-1/2 top-7
        ...
        <div className="mb-4 flex justify-center">
          <div className="w-full max-w-[480px] inline-flex flex-wrap items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 bg-slate-900/60 border border-slate-700/60 rounded-xl shadow">
            <span className="text-slate-400 text-xs sm:text-sm">VH played:</span>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm table-fixed sm:table-auto">
              <thead className="bg-black/30 backdrop-blur">
                <tr>
                  <th className="w-[72px] sm:min-w-[160px] text-left px-3 sm:px-4 py-3 text-slate-400">
                    ...
                    <th key={ci} className={`px-1.5 sm:px-3 py-1.5 text-center ${activeCol === ci ? "bg-amber-500/5" : ""}`}>
                      <div className="flex flex-col items-center gap-1">
                        <img
                          src={VH_ICONS[c]}
                          alt={c}
                          className="w-7 h-7 sm:w-10 sm:h-10 rounded-full object-cover border border-slate-700/60"
                          onError={(e) => { e.currentTarget.style.display = "none"; }}
                        />
                        <span className={`hidden sm:block text-base font-semibold ${activeCol === ci ? "text-amber-300" : ""}`}>
                          {c}
                        </span>
                      </div>
                    </th>
                      ...
                      {state.cols.map((_, ci) => {
                        ...
                        return (
                          <td key={ci} className={`px-1 sm:px-2 md:px-3 py-2 text-center ${active ? "bg-amber-500/5" : ""}`}>
                            <button
                              className={`w-12 h-9 sm:w-20 sm:h-11 rounded-xl border grid place-items-center select-none transition ${
                                active
